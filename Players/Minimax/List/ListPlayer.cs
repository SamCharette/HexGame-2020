...
namespace Players.Minimax.List
{
    public class ListPlayer : MinimaxPlayer
    {
        ...
        public Queue<ListNode> PossibleMovesToMake { get; set; }
        public int MaxProcessors = Environment.ProcessorCount / 2;
        public int CurrentProcessorsInUse = 0;
        ...
        public void StartThreadedSearch(ListNode move,  ListNode choice, int bestScore)
        {
            NodesChecked = 0;
        ...
        public override Tuple<int, int> SelectHex(Tuple<int, int> opponentMove)
        {
            ...
            if (PossibleMovesToMake != null)
            {
                ...
                while (PossibleMovesToMake.Count > 0)
                {
                    if (CurrentProcessorsInUse < MaxProcessors)
                    {
                        CurrentProcessorsInUse++;
                        var thread = new Thread(() => StartThreadedSearch(PossibleMovesToMake.Dequeue(), choice, bestScore));    
                        thread.Start();
                    }
                }
        ...
        private int LetMeThinkAboutIt(int depth, int alpha, int beta, bool isMaximizing)
        {
            ...
            {
                possibleMoves.CopyTo(Memory.Board
                    .OrderBy(x => x.RandomValue)
                    .Where(x => x.Owner == Common.PlayerType.White).ToArray(), 0);
            }
        ...
        private Queue<ListNode> PossibleMoves( bool isMaximizing)
        {
            ...
            var possibleMoves = new System.Collections.Generic.Queue<ListNode>();

            if (myBestPathFromHere != null)
            {
                foreach (var move in myBestPathFromHere)
                {
                    possibleMoves.Enqueue(move);
                }
            }
            ...
            {
                foreach (var move in opponentBestPathFromHere)
                {
                    possibleMoves.Enqueue(move);
                }
            }
            ...
            //                && possibleMoves.All(y => y != x)).ToArray(), possibleMoves.Count);
